...
@vertex fn vertexMain(input: VertexInput) -> VertexOutput {
    ...
    output.vNormal = vertex.normal.xyz;
    output.vUv = vertex.uv.xy;
...
@fragment fn fragmentMain(input: VertexOutput) -> FragmentOutput {
    ...
    if (albedoMapIndex != -1) {
        albedo *= textureSampleLevel(AlbedoMaps, textureSampler, uv.xy, albedoMapIndex, 0); // TODO: No mipmapping
    }

    var normal: vec3f = input.vNormal;
    if (normalMapIndex != -1) {
        let normalSample = textureSampleLevel(NormalMaps, textureSampler, uv.xy, normalMapIndex, 0).xyz * 2.0 - 1.0; // TODO: No mipmapping
