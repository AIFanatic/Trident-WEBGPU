const compose = (position: number[], quaternion: number[], scale: number[]) => {
...
export const applyRotationFromQuat = (transform: number[], rotation: number[]): number[] => {
    const rotationMatrix = compose([0, 0, 0], rotation, [1,1,1]);
    return multiplyMatrices(rotationMatrix, transform);
}

export const translate = (m: number[], v: number[]): number[] => {
    return [
        1, 0, 0, v[0],
        0, 1, 0, v[1],
        0, 0, 1, v[2],
...
export const scale = (m: number[], v: number[]) => {
    return [
        v[0], 0, 0, 0,
        0, v[1], 0, 0,
        0, 0, v[2], 0,
