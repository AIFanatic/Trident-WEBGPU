...
class MaterialPbrMetallicRoughness {
	baseColorFactor				:  number[];
	baseColorTexture			:  TextureInfoBase | null;
	metallicFactor				:  number;
	roughnessFactor				:  number;
	metallicRoughnessTexture	:  TextureInfoBase | null;
	extensions					:  any;
	extras						:  any;
	...
	private async postProcess() {
		...
		if (this._glTF.textures !== undefined) {
			for (let i = 0; i < this._glTF.textures.length; i++) {
				...
				await this.glTF.textures[i].createTexture();
			}
		}
		// 挂载 Mesh
		...
		if (this._glTF.nodes !== undefined) {
			...
			if (this._glTF.scenes !== undefined) {
				for (let i = 0; i < this._glTF.scenes.length; i++) {
                    if (!this._glTF.scenes[i].nodes) continue;

					this.glTF.scenes[i] = new Scene(this._glTF.scenes[i], this.glTF);
					this.glTF.scenes[i].boundingBox = new BoundingBox();
					const nodeMatrices : mat4[] = new Array(this.glTF.nodes.length).fill(null).map(() => mat4.create());
		...
		const loadBuffer : Promise<boolean> = new Promise<boolean>(async (resolve) => {
			if (this._glTF.buffers) {
				...
				for (const bufferInfo of this._glTF.buffers) {
					try {
                        const base64BufferMagic = "data:application/octet-stream;base64,";
                        if (bufferInfo.uri && bufferInfo.uri.includes(base64BufferMagic)) {
                            const base64Data = bufferInfo.uri.slice(bufferInfo.uri.indexOf(base64BufferMagic) + base64BufferMagic.length, bufferInfo.uri.length);
                            bufferPromises.push(new Promise(resolve => {
                                resolve(Uint8Array.from(atob(base64Data), c => c.charCodeAt(0)).buffer);
                            }))
                        }
                        else {
		...
		const loadImage : Promise<boolean> = new Promise<boolean>(async (resolve) => {
			if (this._glTF.images) {
				...
				for (const imageInfo of this._glTF.images) {
					try {
                        const base64ImageMagic = "data:image/png;base64,";
                        if (imageInfo.uri && imageInfo.uri.includes(base64ImageMagic)) {
                            const base64Data = imageInfo.uri.slice(imageInfo.uri.indexOf(base64ImageMagic) + base64ImageMagic.length, imageInfo.uri.length);
                            imagePromises.push(new Promise(resolve => {
                                const arrayBuffer = Uint8Array.from(atob(base64Data), c => c.charCodeAt(0)).buffer;
                                resolve(createImageBitmap(new Blob([arrayBuffer])));
                            }))
                        }
                        else {
