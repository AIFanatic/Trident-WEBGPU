import styles from "./resources/ColorPicker.css";

export class ColorPicker {
    private HSL = [
        { pos: 0, color: this.hexToRgb('#ff0000') }, // red
        { pos: 17, color: this.hexToRgb('#ffff00') }, // yellow
        { pos: 33, color: this.hexToRgb('#00ff00') }, // green
        { pos: 50, color: this.hexToRgb('#00ffff') }, // cyan
        { pos: 67, color: this.hexToRgb('#0000ff') }, // blue
        { pos: 83, color: this.hexToRgb('#ff00ff') }, // magenta
        { pos: 100, color: this.hexToRgb('#ff0000') }  // red
    ];

    public readonly container: HTMLDivElement;
    private colorCanvas: HTMLDivElement;
    private colorCanvasPicker: HTMLDivElement;
    private color: HTMLDivElement;
    private hue: HTMLInputElement;
    private alpha: HTMLInputElement;
    private colorInput: HTMLDivElement;
    private colorInputR: HTMLInputElement;
    private colorInputG: HTMLInputElement;
    private colorInputB: HTMLInputElement;
    private colorInputA: HTMLInputElement;

    public onColorChanged = (color, alpha) => { };
    ...
    constructor(r = 0, g = 0, b = 0, a = 255) {
        ...
        this.hue.addEventListener("input", event => { this.update() });
        this.alpha.addEventListener("input", event => { this.update() });
        ...
        window.addEventListener("pointerup", event => { isMouseDown = false; })
        window.addEventListener("pointermove", event => { this.updatePicker(isMouseDown, event.clientX, event.clientY) });

        this.colorInputR.addEventListener("change", event => { this.colorChanged() });
        this.colorInputG.addEventListener("change", event => { this.colorChanged() });
        this.colorInputB.addEventListener("change", event => { this.colorChanged() });
        this.colorInputA.addEventListener("change", event => { this.colorChanged() });

        // TODO: Some time to render
        setTimeout(() => {
            this.colorChanged();
        }, 10);
    }
    ...
    update() {
        ...
        this.colorInputR.value = format(color.r); this.colorInputG.value = format(color.g); this.colorInputB.value = format(color.b);
    ...
    setPosition(x, y) { Object.assign(this.container.style, { left: `${x}px`, top: `${y}px` }) }
    setColorRGB(r, g, b, a) { this.colorInputR.value = `${r / 255}`, this.colorInputG.value = `${g / 255}`, this.colorInputB.value = `${b / 255}`, this.colorInputA.value = `${a / 255}`, this.colorChanged(); }
    setColorHEX(hex) { const c = this.hexToRgb(hex); this.setColorRGB(c.r, c.g, c.b, 255) }
    disableColor() { [this.colorInputR, this.colorInputG, this.colorInputB, this.colorCanvas, this.hue].forEach(element => element.classList.add("disabled")); }
    enableColor() { [this.colorInputR, this.colorInputG, this.colorInputB, this.colorCanvas, this.hue].forEach(element => element.classList.remove("disabled")); }
    disableAlpha() { [this.colorInputA, this.alpha].forEach(element => element.classList.add("disabled")); }
    enableAlpha() { [this.colorInputA, this.alpha].forEach(element => element.classList.remove("disabled")); }
}