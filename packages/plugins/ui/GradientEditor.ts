...
import styles from "./resources/GradientEditor.css";
...
export class GradientEditor {
    ...
    private pickersList: HTMLElement[];
    private alphasContainer: HTMLDivElement;
    private gradient: HTMLDivElement;
    private colorsContainer: HTMLDivElement;
    private settingsInputLocation: HTMLInputElement;
    private settingsButtonDelete: HTMLButtonElement;
    private mouse: {down: boolean, x: number};
    private selectedPicker: HTMLDivElement;

    public onChanged = (gradient: Gradient) => {};
    public currentGradient: string = "";

    constructor(defaultGradient?: Gradient) {
        // Inject styles
        ...
        [this.alphasContainer, this.colorsContainer].forEach(gradientContainer => gradientContainer.addEventListener("pointerdown", event => { this.mouse = { down: true, x: event.clientX }; this.onPointerDown(gradientContainer, event.target) }));

        window.addEventListener("pointerup", event => { this.mouse = { down: false, x: event.clientX }; });
        window.addEventListener("pointermove", event => { this.mouse = { down: this.mouse.down, x: event.clientX }; this.onPointerMove() });
    ...
    onPointerDown(container, target) {
        ...
        if (!target.classList.contains("picker")) {
            const type = target === this.alphasContainer ? "alpha" : "color";
            const t = (this.mouse.x - rect.left) / rect.width;
            console.log(this.mouse.x, rect.left, rect.width, t)
            {
                const x = 10;
                const t = (x - rect.left) / rect.width;
                console.log(t, rect.left, rect.width)
            }
            this.selectedPicker = this.addPicker(type, {r:0,g:0,b:0,a:255}, t);
        }

        const selectedPicker = this.selectedPicker;

        this.colorPicker.onColorChanged = (color, alpha) => {
            selectedPicker.dataset.color = JSON.stringify({ r: color.r, g: color.g, b: color.b, a: alpha });
            selectedPicker.style.background = `rgba(${color.r}, ${color.g}, ${color.b}, ${alpha / 255})`;
            this.updatePickerGradient();
        }
    ...
    addPicker(type: "alpha" | "color", color: {r: number, b: number, g: number, a: number}, t: number): HTMLDivElement {
        ...
        // picker.dataset.color = JSON.stringify({ r: 0, g: 0, b: 0, a: 255 });
