...
export class PathTracer extends GPU.RenderPass {
    public name: string = "PathTracer";

    private shader: GPU.Compute;
    private needsMeshUpdate = true;

    public samples: number = 4;
    public bounces: number = 4;

    private outputTexture: GPU.RenderTextureStorage2D;   // compute output
    private accumulation: GPU.RenderTextureStorage2D;    // persistent accumulation

    private uniformsBuffer = new ArrayBuffer(112);

    private UniformsViews = {
    ...
    private currentMeshCount = 0;

    public customBlender: GPU.Shader;
    public customBlenderGeometry: Geometry;
    public customBlenderTarget: GPU.RenderTexture;
    ...
    private updateMeshBuffers() {
        ...
        let meshesBuffer: number[] = [];
        let materialsBuffer: number[] = [];
        ...
        for (const renderable of renderables) {
            ...
            const vertexBase = vertexBuffer.length / 4; // current vertex count (vec4 stride)
    ...
    private prevCameraRotation = new Mathf.Quaternion();
    private prevCameraPosition = new Mathf.Vector3();
