import { Geometry, Mathf, PBRMaterial, Texture as TridentTexture, IndexAttribute, VertexAttribute } from "@trident/core";
import { GLTFLoader, AccessorComponentType } from './GLTFLoader_Minimal';
export class GLTFParser {
    static TextureCache = new Map();
    static async getTexture(textures, textureInfo) {
        if (!textures || !textureInfo)
            return undefined;
    ...
    static async parsePrimitive(primitive, textures) {
        ...
        if (primitive.attributes.POSITION)
            geometry.attributes.set("position", new VertexAttribute(new Float32Array(primitive.attributes.POSITION.bufferView.data)));
        if (primitive.attributes.NORMAL)
            geometry.attributes.set("normal", new VertexAttribute(new Float32Array(primitive.attributes.NORMAL.bufferView.data)));
        if (primitive.attributes.TEXCOORD_0)
            geometry.attributes.set("uv", new VertexAttribute(new Float32Array(primitive.attributes.TEXCOORD_0.bufferView.data)));
