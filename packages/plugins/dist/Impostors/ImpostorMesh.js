...
export class ImpostorMesh extends Mesh {
    ...
    albedoTexture;
    normalTexture;
    async Create(objects, atlasResolution = 2048, atlasTiles = 12) {
        let radius = 0;
        ...
        for (const object of objects) {
            ...
            if (object.material) {
                if (object.material.params.albedoColor)
                    shader.SetArray("albedoColor", object.material.params.albedoColor.elements);
                if (object.material.params.albedoMap)
                    shader.SetTexture("albedoMap", object.material.params.albedoMap);
            }
        ...
        for (var x = 0; x < frames; x++) {
            for (var y = 0; y < frames; y++) {
                var vec = new Vector2(x / framesMinusOne * 2.0 - 1.0, y / framesMinusOne * 2.0 - 1.0);
    ...
    async createImpostorMesh(atlasTiles, albedoTexture, normalTexture) {
