...
export class Atlas {
    ...
    AddTexture(texture) {
        ...
        if (!node || !node.rect)
            throw Error("Failed to insert texture into atlas");
        const region = {
        ...
        RendererContext.CopyTextureToBufferV2({ texture: texture, mipLevel: 0, origin: [0, 0, 0] }, { buffer: this.buffer, offset: offset, bytesPerRow: this.size * 4 });
    ...
    UpdateRegionData() {
        ...
        for (const region of this.regions)
            regionData.push(region.uvOffset[0], region.uvOffset[1], region.uvSize[0], region.uvSize[1]);
