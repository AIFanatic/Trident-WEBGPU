...
import { AABB, Requirements, Result, Range } from "./nvcluster";
...
class Split {
    axis = -1;
    position = Number.MAX_VALUE; // index of first item in the right node
    cost = Number.MAX_VALUE;
    valid() { return this.axis !== -1; }
    lessThan(other) { return this.cost < other.cost; }
    copy(other) {
...
function splitAtMedianUntil(spatialElements, // Original definition of the input spatial elements
partitionSides, // Partition identifier (left = 1, right = 0) for each element, used to partition the sorted element lists
nodeSortedElementIndicesPerAxis, // Sorted element indices along each axis for the current node
maxElementsPerNode, // Maximum number of elements allowed per node, used to stop the recursion
nodeStartIndex, // Starting index of the node in the complete sorted lists of elements
perNodeElementRanges // Output ranges of the nodes (in the sorted element lists) created by the recursive split
) {
...
function sumAdjacencyWeightsAtSplit(graph, // adjacency graph storing the connections for each node
connectionIndicesInSortedElements, // Flattened list of the connections for each node in the graph. The indices represent the connected elements in the sorted list of elements.
node, // Node to split, whose connections are defined by its offset and count in connectionIndicesInSortedElements
elementIndexInNodeRange, // Index at which the node would be split, as a value between 0 and node.count
elementIndex // Index of the element in the sorted list of elements corresponding to the split element
) {
    let result = 0.0;
...
function splitCost(AlignBoth, input, splitWeights, // Cost of the graph cut for each element in the node
leftAabb, // Bounding box of the elements on the left side of the split
rightAabb, // Bounding box of the elements on the right side of the split
nodeSize, // Number of elements in the node
splitPositionFromLeft // Location of the split, as the index of the first element in the right side of the split
) {
...
function findBestSplit(Axis, AlignBoth, input, node, sortedIndicesThisAxis, nodeLeftBoxes, // size = node.count
nodeRightBoxes, // size = node.count
connectionIndicesInSortedElements, deltaWeights, // size = node.count
splitWeights, // size = node.count
splitOut) {
    ...
    if (input.graph) {
        for (let i = 0; i < N; i++) {
            ...
            deltaWeights[i] = sumAdjacencyWeightsAtSplit(input.graph, connectionIndicesInSortedElements, node, i, elemIndex);
        }
        let runningSum = 0.0;
        for (let i = 0; i < N; i++) {
...
function splitNode(input, // Input elements and adjacency graph
temporaries, // Temporary storage for the split node function
node, // Range of elements in the node to split
outNodes, // Output nodes storage where child nodes will be added
outNodesAlloc // Current number of nodes in the output
) {
...
export function clusterize(input, clusters // Output set of clusters
) {
