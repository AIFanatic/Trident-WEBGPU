...
import { AABB, Input, Range, SpatialElements, Result as ResultNVCluster, Graph, generateClusters } from "./nvcluster";
import { ClusterStorage, SegmentedClusterStorage, generateSegmentedClusters } from "./nvcluster_storage";
import { Result, assert, createArrayView, number_to_uvec3, resizeArray, vec3_to_number } from "./nvclusterlod_common";
...
export function nvclusterlodMeshGetRequirements(info) {
...
//   }
// }
export function exclusive_scan_impl(input, init, op) {
    let sum = init;
    const output = [];
...
function decimateClusterGroups(current, triangleClusters, clusterGrouping, lodLevelDecimationFactor) {
    ...
    for (let clusterGroupIndex = 0; clusterGroupIndex < clusterGrouping.groups.clusterRanges.length; clusterGroupIndex++) {
        ...
        const ret = Meshoptimizer.meshopt_simplifyWithAttributes(decimatedTriangleVertices.length * 3, new Uint32Array(vec3_to_number(clusterGroupTriangleVertices)), clusterGroupTriangleVertices.length * 3, inputMesh.vertices, // getVertex(inputMesh, 0),
        inputMesh.vertexCount, inputMesh.vertexStride, null, 0, null, 0, new Uint8Array(result.globalLockedVertices), desiredTriangleCount * 3, targetError, options);
        ...
        if (desiredTriangleCount < simplifiedTriangleCount) {
            ...
            const ret_shadowIndexBuffer = Meshoptimizer.meshopt_generateShadowIndexBuffer(new Uint32Array(vec3_to_number(clusterGroupTriangleVertices)), clusterGroupTriangleVertices.length * 3, inputMesh.vertices, // getVertex(inputMesh, 0),
            inputMesh.vertexCount, 3, // vertex_size, for interleaved this stays at 3 * 4
            inputMesh.vertexStride);
            positionUniqueTriangleVertices = number_to_uvec3(ret_shadowIndexBuffer);
            const ret = Meshoptimizer.meshopt_simplifyWithAttributes(decimatedTriangleVertices.length * 3, new Uint32Array(vec3_to_number(positionUniqueTriangleVertices)), positionUniqueTriangleVertices.length * 3, inputMesh.vertices, // getVertex(inputMesh, 0),
            inputMesh.vertexCount, inputMesh.vertexStride, null, 0, null, 0, new Uint8Array(result.globalLockedVertices), desiredTriangleCount * 3, targetError, options);
...
function generateTriangleClusters(decimatedClusterGroups, clusterConfig, output) {
    ...
    let clusteringResult = generateSegmentedClusters(triangleClusterInput, decimatedClusterGroups.groupTriangleRanges, decimatedClusterGroups.groupTriangleRanges.length, output.clustering);
...
class ClusterGroups {
    ...
    groupTriangleCounts = []; // useful byproduct
...
export function nvclusterlodMeshCreate(info, output) {
