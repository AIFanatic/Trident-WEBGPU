...
// Icosahedron data.
class icosahedron {
    static X = 0.525731112119133606;
    static Z = 0.850650808352039932;
    static positions = [
...
class GeometryMesh {
    triangles = [];
    positions = [];
    constructor(triangles, positions) {
...
function subdivide(v0, v1, v2, depth, callback) {
    if (depth == 0) {
        callback(v0, v1, v2);
    }
    else {
        const v01 = normalize(add(v0, v1));
        const v12 = normalize(add(v1, v2));
        const v20 = normalize(add(v2, v0));
...
function makeIcosphereWithCallback(depth, callback) {
...
function makeIcosphere(subdivision) {
...
function getSpherePosition(sphere) {
...
function verifyNodeRecursive(m, h, node) {
    if (node.clusters && node.clusters.isLeafNode === 1) {
        ...
        const clusterRange = m.lodMesh.groupClusterRanges[node.clusters.group];
        for (let i = 0; i < clusterRange.count; i++) {
            const clusterSphere = m.lodMesh.clusterBoundingSpheres[clusterRange.offset + i];
            // #if 0  // For debugging
        ...
        for (let i = 0; i <= node.children.childCountMinusOne; i++) {
            const child = h.nodes[node.children.childOffset + i];
...
function randomPointOnSphere(sphere) {
...
function conservativeErrorOverDistance(transform, boundingSphere, objectSpaceQuadricError) {
...
function traverseChild1(viewInstanceTransform, node, errorOverDistanceThreshold) {
...
function renderCluster1(viewInstanceTransform, quadricError, boundingSphere, errorOverDistanceThreshold) {
    return conservativeErrorOverDistance(viewInstanceTransform, boundingSphere, quadricError) < errorOverDistanceThreshold;
}
function traverseChild2(cameraPosition, node, errorOverDistanceThreshold) {
    return traverseChild1(mat4.makeTranslation(mul(cameraPosition, -1.0)), node, errorOverDistanceThreshold);
}
function renderCluster2(cameraPosition, quadricError, boundingSphere, errorOverDistanceThreshold) {
...
function verifyMutuallyExclusive(m, h, cluster0, node0, cluster1, node1) {
    const cluster0Sphere = generatingSphere(h.groupCumulativeBoundingSpheres, m.lodMesh.clusterGeneratingGroups[cluster0]);
    const cluster1Sphere = generatingSphere(h.groupCumulativeBoundingSpheres, m.lodMesh.clusterGeneratingGroups[cluster1]);
    const cluster0QuadricError = generatingError(h.groupCumulativeQuadricError, m.lodMesh.clusterGeneratingGroups[cluster0]);
    const cluster1QuadricError = generatingError(h.groupCumulativeQuadricError, m.lodMesh.clusterGeneratingGroups[cluster1]);
    const errorOverDistanceThreshold = 0.9999; // near worst case
    for (let i = 0; i < 10; ++i) {
        const testCameraPos = randomPointOnSphere(node0.boundingSphere);
        const begin0 = traverseChild2(testCameraPos, node0, errorOverDistanceThreshold);
        const begin1 = traverseChild2(testCameraPos, node1, errorOverDistanceThreshold);
        const end0 = !renderCluster2(testCameraPos, cluster0QuadricError, cluster0Sphere, errorOverDistanceThreshold);
        const end1 = !renderCluster2(testCameraPos, cluster1QuadricError, cluster1Sphere, errorOverDistanceThreshold);
        const bothVisible = begin0 && !end0 && begin1 && !end1;
...
async function TEST_HierarchyTest_Everything() {
    ...
    const mesh = new LocalizedLodMesh();
    let result = generateLocalizedLodMesh(meshInput, mesh);
    ...
    ASSERT_EQ(result, Result.SUCCESS);
    let clusterNodes = new Array(mesh.lodMesh.clusterTriangleRanges.length).fill(0);
    for (let nodeIndex = 0; nodeIndex < hierarchy.nodes.length; ++nodeIndex) {
        if (hierarchy.nodes[nodeIndex] && hierarchy.nodes[nodeIndex].clusters && hierarchy.nodes[nodeIndex].clusters.isLeafNode === 1) {
            const clusterIndices = mesh.lodMesh.groupClusterRanges[hierarchy.nodes[nodeIndex].clusters.group];
    ...
    for (let groupIndex = mesh.lodMesh.groupClusterRanges.length; (groupIndex--) > 0;) {
        ...
        for (let clusterIndex = groupClusterRange.offset; clusterIndex < groupClusterRange.offset + groupClusterRange.count; clusterIndex++) {
            ...
            if (generatingGroupIndex == ORIGINAL_MESH_GROUP)
                continue;
            for (let generatingGeneratingGroupIndex of groupGeneratingGroups.get(generatingGroupIndex)) {
                ...
                const generatingGeneratingClusterRange = mesh.lodMesh.groupClusterRanges[generatingGeneratingGroupIndex];
                for (let i = 0; i < generatingGeneratingClusterRange.count; i++) {
                    ...
                    verifyMutuallyExclusive(mesh, hierarchy, clusterIndex, hierarchy.nodes[clusterNodes[clusterIndex]], generatingGeneratingClusterIndex, hierarchy.nodes[clusterNodes[generatingGeneratingClusterIndex]]);
                }
            }
        }
    }
...
async function TEST_Hierarchy_AngularError() {
    ...
    const fovValues = [Math.PI * 0.05, Math.PI * 0.3, Math.PI * 0.5, Math.PI * 0.7];
    const pixelValues = [0.5, 1.0, 10.0, 100.0, 1000.0];
