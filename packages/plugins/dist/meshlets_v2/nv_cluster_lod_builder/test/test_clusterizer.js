...
function sortClustersClusterStorage(clustering) {
...
export function sortClustersSegmentedClusterStorage(clustering) {
    // Helper to sort a subarray in place.
    const sortSlice = (array, offset, count, cmp) => {
    ...
    clustering.clusterRanges.forEach(cluster => sortSlice(clustering.clusterItems, cluster.offset, cluster.count, (a, b) => (a < b ? -1 : a > b ? 1 : 0)));
    // Sort cluster ranges within each segment,
    // comparing based on the first item of each cluster.
    clustering.clusterRangeSegments.forEach(segment => sortSlice(clustering.clusterRanges, segment.offset, segment.count, (rangeA, rangeB) => {
...
function TEST_Simple2x2() {
    ...
    // Generate centroids
    let centroids = new Array(boundingBoxes.length).fill(null).map(() => new vec3());
    ...
    const result = generateClusters(input, clustering);
    ...
    const cluster0 = clustering.clusterRanges[0];
    ...
    const cluster1 = clustering.clusterRanges[1];
...
function TEST_Simple2x2Weights() {
    ...
    const graphNodes = [new Range(0, 2), new Range(2, 2), new Range(4, 2), new Range(6, 2)]; // 2 connections each
    ...
    // Generate centroids
    let centroids = new Array(boundingBoxes.length);
    for (let i = 0; i < boundingBoxes.length; i++) {
        centroids[i] = centroid(boundingBoxes[i]);
    }
    // Input structs
    const spatialElements = new SpatialElements();
    spatialElements.boundingBoxes = boundingBoxes;
    spatialElements.centroids = vec3_to_number(centroids);
    spatialElements.elementCount = boundingBoxes.length;
    ...
    let result = generateClusters(input, clustering);
...
function TEST_Segmented2x2() {
    ...
    // input.graph           = nullptr,
    // Clustering
    const clustering = new SegmentedClusterStorage();
    const result = generateSegmentedClusters(input, segments, segments.length, clustering);
    ASSERT_EQ(result, Result.SUCCESS);
    ...
    for (let segmentIndex = 0; segmentIndex < 3; ++segmentIndex) {
        const segment = clustering.clusterRangeSegments[segmentIndex];
        const firstCluster = clustering.clusterRanges[segment.offset + 0];
    ...
    for (let segmentIndex = 0; segmentIndex < 3; ++segmentIndex) {
        ...
        const cluster0 = clustering.clusterRanges[segment.offset + 0];
        ASSERT_EQ(cluster0.count, 2);
        ...
        const cluster1 = clustering.clusterRanges[segment.offset + 1];
        ASSERT_EQ(cluster1.count, 2);
...
function aabb(triangle, positions) {
    let result = new AABB();
    let vertex = positions[triangle[0]];
    result.bboxMin[0] = vertex.x;
    result.bboxMin[1] = vertex.y;
    result.bboxMin[2] = vertex.z;
    result.bboxMax[0] = vertex.x;
    result.bboxMax[1] = vertex.y;
    result.bboxMax[2] = vertex.z;
    for (let i = 1; i < 3; i++) {
...
function makeIcosphereCB(depth, callback) {
...
function makeIcosphere(subdivision) {
    ...
    let positions = new Array(vertexCache.size).fill(null).map(() => new vec3());
...
function TEST_ClustersTestSizes() {
    const mesh = makeIcosphere(3);
    let boundingBoxes = new Array(mesh.triangles.length).fill(null).map(() => new AABB());
...
function TEST_PreSplit() {
    ...
    let boundingBoxes = new Array(mesh.triangles.length);
    for (let i = 0; i < mesh.triangles.length; i++) {
        boundingBoxes[i] = aabb(mesh.triangles[i], mesh.positions);
    }
    let centroids = new Array(boundingBoxes.length);
    ...
    const uniqueItems = new Set(clustering.clusterItems);
    EXPECT_EQ(uniqueItems.size, clustering.clusterItems.length);
    // Validate all items are covered by a range exactly once
    let itemClusterCounts = new Array(clustering.clusterItems.length).fill(0);
    ...
    let clusterSizeCounts = {}; // cluster size -> number of clusters with that size
    for (const range of clustering.clusterRanges) {
        if (!clusterSizeCounts[range.count])
            clusterSizeCounts[range.count] = 0;
