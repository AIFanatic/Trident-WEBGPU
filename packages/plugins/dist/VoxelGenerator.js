...
import { RenderTextureStorage3D } from "../renderer/Texture";
export class VoxelGenerator {
    static async Generate(geometry, scale = new Vector3(1, 1, 1)) {
        ...
        if (!vertices)
            throw Error("Vertices not found");
        if (!normals)
            throw Error("Normals not found");
        if (!uvs)
            throw Error("UVS not found");
        if (!indices)
            throw Error("Indices not found");
        const padding = 8;
