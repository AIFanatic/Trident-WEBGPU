...
export let lightsCSMProjectionMatrix = [];
const getWorldSpaceCorners = (camera, zNearPercentage, zFarPercentage) => {
...
const getLightViewProjections = (camera, lightDirection, numOfCascades, assignmentExponent, shadowDepthPercentage, zMult) => {
    if (camera == null) {
        return [new Matrix4()];
    }
    ...
    for (let i = 0; i < numOfCascades; ++i) {
        ...
        if (minZ < 0)
            minZ *= zMult; // become even more negative :)
        else
            minZ /= zMult; // reduce value
        if (maxZ < 0)
            maxZ /= zMult; // become less negative :)
        else
            maxZ *= zMult; // increase value
