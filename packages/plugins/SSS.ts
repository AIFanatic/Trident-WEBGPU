...
export interface SSSDispatchData {
    waveCountX: number;        // Dispatch X
    waveCountY: number;        // Dispatch Y
    waveCountZ: number;        // Dispatch Z
    waveOffsetX: number;       // g_params.WaveOffset.x  (pixels)
    waveOffsetY: number;       // g_params.WaveOffset.y  (pixels)
}

export interface SSSDispatchList {
    lightCoordShader: Float32Array;  // [x_px, y_px, depth, dirSign]
    dispatches: SSSDispatchData[];   // up to 8
}

/**
 * Port of Bend::BuildDispatchList
 *
 * @param lightProj   clip-space light (pre-divide) — float4( * , w ). For dir lights use float4( dirVS, 0 ).
 * @param viewport    [width, height]
 * @param minBounds   [minX, minY] inclusive render bounds in pixels
 * @param maxBounds   [maxX, maxY] inclusive render bounds in pixels
 * @param expandedZ   true if your API expects expanded Z (GL -1..+1 → depth 0..1). WebGPU/D3D: false.
 * @param waveSize    must match @workgroup_size.x (64)
 */
export function buildSSSDispatchList(
    lightProj: IVec4,
    viewport: IVec2,
    minBounds: IVec2,
    maxBounds: IVec2,
    ...
    const lightXYi: IVec2 = [Math.floor(lightX + 0.5), Math.floor(lightY + 0.5)];

    // Inclusive bounds relative to the light
    const biased = [
        minX - lightXYi[0],                 // left
        -(maxY - lightXYi[1]),              // bottom (note Y flip)
        maxX - lightXYi[0],                 // right
        -(minY - lightXYi[1]),              // top
    ] as const;

    // Process 4 quadrants around the light
    for (let q = 0; q < 4; q++) {
        const vertical = q === 0 || q === 3;

        // bounds in waves (ceil-ish with biases)
        ...
        if ((bx1 - bx0) > 0 && (by1 - by0) > 0) {
            ...
            const disp: SSSDispatchData = {
                waveCountX: waveSize,                    // waves along X dimension (same as Bend)
                waveCountY: (bx1 - bx0),
                waveCountZ: (by1 - by0),
            ...
            const pushIfValid = (d: SSSDispatchData) => {
                if (d.waveCountY > 0 && d.waveCountZ > 0) result.dispatches.push(d);
            };

            if (axis_delta > 0) {
                const disp2: SSSDispatchData = { ...disp }; // split
    ...
    // Scale offsets by wave size (shader expects pixel offsets)
...
export class SSSRenderPass extends GPU.RenderPass {
    public name: string = "SSSRenderPass";
    public output: GPU.RenderTexture;

    private light: Components.Light;
    private compute: GPU.Compute;
    private blendShader: GPU.Shader;
    private blendGeometry: Geometry;
    private blendOutput: GPU.Texture;

    /* Settings */
    public Enabled: boolean = true;
    public BlendShadowStrength: number = 0.5;

    // Visual configuration
    public SurfaceThickness: number = 0.005;
    public BilinearThreshold: number = 0.02;
    public ShadowContrast: number = 4.0;

    public IgnoreEdgePixels: boolean = false;
    public UsePrecisionOffset: boolean = false;
    public BilinearSamplingOffsetMode: boolean = false;

    public DebugOutputEdgeMask: boolean = false;
    public DebugOutputThreadIndex: boolean = false;
    public DebugOutputWaveIndex: boolean = false;

    // Culling / Early out
    public DepthBounds: Mathf.Vector2 = new Mathf.Vector2(0.0, 1.0);
    public UseEarlyOut: boolean = false;

    // Renderer-specific
    public FarDepthValue: number = 1.0;
    public NearDepthValue: number = 0.0;

    private SettingsArray = new ArrayBuffer(112);
    public Settings = {
    ...
    constructor(light: Components.Light) {
        super();
        this.light = light;
    }
    ...
    private WaveOffsetBuffer: GPU.DynamicBuffer;
    private _sssDispatchList: SSSDispatchList;
    private settingsBuffer: GPU.Buffer;
    public async preFrame(resources: GPU.ResourcePool) {
        ...
        this._sssDispatchList = buildSSSDispatchList(
          ...
          [0, 0],                               // min render bounds
          [gDepthTex.width - 1, gDepthTex.height - 1],  // max render bounds
          /* expandedZ = */ false,               // WebGPU/D3D-style depth
          /* waveSize  = */ 64
        );
        ...
        // const waveSize = 64;
        // const waveTilesX = Math.ceil(gDepthTex.width / waveSize);
        // const waveTilesY = Math.ceil(gDepthTex.height / waveSize);
        // const offsetX = -Math.floor((waveTilesX * waveSize) * 0.5);
        // const offsetY = -Math.floor((waveTilesY * waveSize) * 0.5);
        // this.WaveOffsetBuffer.SetArray(new Int32Array([offsetX, offsetY]), 0 * 256 * 4);
        ...
        this.Settings.InvDepthTextureSize.set([1.0 / gDepthTex.width, 1.0 / gDepthTex.height]);
    ...
    public async execute(resources: GPU.ResourcePool) {
        ...
        // Compute SSS
        // Run all dispatch tiles for this light
        ...
        // const WAVE_SIZE = 64;
        // const workgroupCountX = WAVE_SIZE;
        // const workgroupCountY = Math.ceil(gDepthTex.height / WAVE_SIZE);
        // const workgroupCountZ = Math.ceil(gDepthTex.width / WAVE_SIZE);
        // GPU.ComputeContext.BeginComputePass(this.name, true);
        // GPU.ComputeContext.Dispatch(this.compute, workgroupCountX, workgroupCountY, workgroupCountZ);
        // GPU.ComputeContext.EndComputePass();

        // Blend with lighting output
        const LightingPassOutput = resources.getResource(GPU.PassParams.LightingPassOutput);
