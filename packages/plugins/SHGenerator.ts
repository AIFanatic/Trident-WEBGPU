// Based on https://zznewclear13.github.io/posts/calculate-spherical-harmonics-using-compute-shader/
// Optmized to do all 9 coefficients in one compute pass.

import { GPU, Mathf } from "@trident/core";

export class SHGenerator {
    private compute: GPU.ShaderCompute;
    private format: GPU.TextureFormat = "rgba16float";

    private initialized = false;

    public async Initialize(size: number) {
    ...
    public async FromCubemap(cubemap: GPU.CubeTexture) {
        ...
        // 9 vec4f = 9 * 4 floats = 36 floats per group
