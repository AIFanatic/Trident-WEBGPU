...
export class TilesGenerator {
    ...
    private static ctx = TilesGenerator.canvas.getContext("2d", { willReadFrequently: true })!;

    public static imageBitmapToImageData(bitmap: ImageBitmap): ImageData {
    ...
    private static copy(img: ImageData): ImageData {
    ...
    private static resize(img: ImageData, w: number, h: number, filter: "nearest" | "box" | "bilinear"): ImageData {
    ...
    private static tilesForLevel(img: ImageData, lod: number, tileW: number, tileH: number, pad: number): Tile[] {
        ...
        for (let ty = 0; ty < tiles_y; ty++) {
            for (let tx = 0; tx < tiles_x; tx++) {
                ...
                tiles.push({ lod: lod, tile_x: tx, tile_y: ty, data: tile });
            }
        }
        return tiles;
    }

    public static generate(img: ImageData, tileW: number, tileH: number, pad = 0, filter: "nearest" | "box" | "bilinear" = "box"): Tile[] {
        ...
        while (true) {
            ...
            current_width = nw; current_height = nh; lod++;
        }
        return all;
    }
}
