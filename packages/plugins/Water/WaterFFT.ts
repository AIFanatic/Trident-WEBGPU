import { Component, Components, GameObject, Geometry, GPU, IndexAttribute, Renderer, Runtime, Scene, VertexAttribute } from "@trident/core";
...
function FFTWGSL(horizontal: boolean): string {
    const axisIndex = horizontal ? "input.position.x" : "input.position.y";
    const evenCoord = horizontal
        ? "vec2i(i32(evenIndex), i32(input.position.y))"
        : "vec2i(i32(input.position.x), i32(evenIndex))";
    const oddCoord = horizontal
        ? "vec2i(i32(evenIndex + transformSize * 0.5), i32(input.position.y))"
        : "vec2i(i32(input.position.x), i32(evenIndex + transformSize * 0.5))";
...
class WaterFFTRenderPass extends GPU.RenderPass {
    public name = "WaterFFTRenderPass";

    private readonly resolution = 64;
    private readonly oceanSize = 250;
    private readonly choppiness = 0.35;
    private readonly windX = 10;
    private readonly windY = 10;
    private readonly displacementScale = 0.25;
    private readonly roughness = 0.025;
    private readonly refraction = 0.035;

    public readonly waterGeometries = new Map<WaterFFT, WaterFFTInfo>();

    private seedPhaseShader: GPU.Shader;
    private initialSpectrumShader: GPU.Shader;
    private phaseShader: GPU.Shader;
    private spectrumShader: GPU.Shader;
    private fftHorizontalShader: GPU.Shader;
    private fftVerticalShader: GPU.Shader;
    private normalShader: GPU.Shader;
    private renderShader: GPU.Shader;

    private initialSpectrum: GPU.Texture;
    private phaseA: GPU.Texture;
    private phaseB: GPU.Texture;
    private spectrum: GPU.Texture;
    private transformPing: GPU.Texture;
    private transformPong: GPU.Texture;
    private displacementMap: GPU.Texture;
    private normalMap: GPU.Texture;

    private albedoClone: GPU.Texture;
    private depthClone: GPU.Texture;

    private nearestClampSampler: GPU.TextureSampler;
    private nearestRepeatSampler: GPU.TextureSampler;
    private linearRepeatSampler: GPU.TextureSampler;

    private phaseAIsCurrent = true;
    private generatedInitialTextures = false;
    private previousTime = 0;

    private readonly simParams = new Float32Array(8);
    private readonly fftParams = new Float32Array(4);
    private readonly renderParams = new Float32Array([
        this.displacementScale, this.roughness, this.refraction, 1 / (this.resolution * this.resolution),
        0.005, 0.025, 0.055, 1,
        0.0, 0.32, 0.36, 1,
    ]);
    ...
    private createFFTTexture(name: string): GPU.Texture {
    ...
    private updateScreenClones(currentAlbedo: GPU.Texture, currentDepth: GPU.Texture) {
    ...
    private drawFullscreen(name: string, target: GPU.Texture, shader: GPU.Shader) {
        GPU.RendererContext.BeginRenderPass(name, [{ target, clear: true }], undefined, true);
        GPU.RendererContext.DrawVertex(shader, 3);
        GPU.RendererContext.EndRenderPass();
    }
}

export class WaterFFT extends Component {
    public static type = "@trident/plugins/WaterFFT";
    public runInEditMode = true;

    private static renderPass: WaterFFTRenderPass;
    private static renderPassScene: Scene;
