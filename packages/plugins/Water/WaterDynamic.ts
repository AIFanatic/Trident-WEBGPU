...
import { GameObject, Geometry, IndexAttribute, VertexAttribute, Components, Component, Renderer, GPU, Scene, Runtime, Mathf } from "@trident/core";
...
class WaterRenderPass extends GPU.RenderPass {
    ...
    private collisionTexture: GPU.RenderTexture; // Input collisions/land
    private oldSimulationTexture: GPU.RenderTexture; // Output
    private simulationTexture: GPU.RenderTexture; // Output
    private simulationTextureRenderTarget: GPU.RenderTexture; // Output

    private simulateShader: GPU.Shader;
    private renderShader: GPU.Shader;

    public waterGeometries: WaterInfo[] = [];
    ...
    public async execute(resources: GPU.ResourcePool) {
        ...
        const params = new Float32Array([
            512, 0, 0, 0,
            performance.now() / 1000, 0, 0, 0
        ]);
        this.simulateShader.SetArray("params", params);
        this.renderShader.SetArray("params", params);

        
        GPU.RendererContext.BeginRenderPass(this.name, [ { target: this.simulationTextureRenderTarget, clear: true } ], undefined, true);
...
export class WaterDynamic extends Component {
