import { Components, GPU, Mathf } from "@trident/core";
import { Terrain } from "@trident/plugins/Terrain/Terrain";

export class TerrainProceduralProp extends Components.Component {
    public terrain: Terrain;
    private propPlacementCompute: GPU.ShaderCompute;

    private instanceCounts: GPU.Buffer;

    public AddProps() {
        // { name: "CLIFF", albedoColor: new Mathf.Color(0.22, 0.2, 0.2)},
        // { name: "DIRT", albedoColor: new Mathf.Color(0.6, 0.5, 0.4)},
        // { name: "GRASS", albedoColor: new Mathf.Color(0.15, 0.3, 0.1)},
        // { name: "SNOW", albedoColor: new Mathf.Color(0.9, 0.9, 0.9)},
        // { name: "SAND", albedoColor: new Mathf.Color(0.8, 0.7, 0.6)},
        // { name: "WATER", albedoColor: new Mathf.Color(0.0, 0.05, 0.1)},
    }
    ...
    public async Generate(terrainTexture, materialIdMap) {
        if (!this.terrain) throw Error("No terrain assigned");
        if (this.terrain.terrainData.paintPropData.length === 0) throw Error("No props");

        await this.Load();

        // 1) RESET the atomic before each dispatch — Load() only zeroes it once,
        // so subsequent Generate() calls keep accumulating into stale slots.
        this.instanceCounts.SetArray(new Uint32Array([0]));
        ...
        // 2) Read back the atomic count → set _instanceCount to ACTUAL placements only
