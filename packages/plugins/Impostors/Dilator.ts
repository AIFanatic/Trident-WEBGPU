...
export class Dilator {
    public static async Dilate(texture: GPU.RenderTexture, iterations = 32): Promise<GPU.RenderTexture> {
        ...
        GPU.RendererContext.CopyTextureToTextureV3({ texture }, { texture: a }); // seed once
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            [read, write] = [write, read]; // ping-pong, no copies
        }
        GPU.Renderer.EndRenderFrame();

        (read === a ? b : a).Destroy(); // free the spare
        return read;
    }
}