...
async function Application() {
    ...
    function step(meshlets: Meshlet[], ctx: CanvasRenderingContext2D) {
        ...
        let groups = [meshlets];
        const nparts = Math.ceil(meshlets.length / 4);
        if (nparts > 1) {
            groups = MeshletGrouper.group(meshlets, nparts);
        }
        // console.log(`nparts: ${nparts}, groups: ${groups.length}, meshlets: ${meshlets.length}`);
        let iteration = 0;
        for (let i = 0; i < groups.length; i++) {
            ...
            if (simplified.meshlet.vertices.length === cleaned.vertices.length && simplified.meshlet.indices.length === cleaned.indices.length) {
                console.log("vertices", simplified.meshlet.vertices);
                console.log("indices", simplified.meshlet.indices);
                console.warn(simplified.meshlet.vertices.length, cleaned.vertices.length, simplified.meshlet.indices.length, cleaned.indices.length)
                throw Error("ERGERGER");
            }
            console.log(simplified.meshlet.vertices.length, cleaned.vertices.length, simplified.meshlet.indices.length, cleaned.indices.length)
            const simplified2 = FQMR.setSimplifyRebuilt(simplified.meshlet, 100000000, false, false);
            ...
            // console.log(simplified.meshlet.indices.length / 3, simplified2.indices.length / 3, simplified3.indices.length / 3);
            ...
            // const parts = Math.ceil(simplified.meshlet.indices.length / 3 / 128);
            const parts = Math.ceil(group.length / 2);
            let splits = [simplified.meshlet];
            if (parts > 1) {
                splits = MeshletGrouper.split(simplified.meshlet, parts);
            }
            // const splits = MeshletGrouper.split(simplified.meshlet, parts);
            
            // const splits = MeshletCreator.build(simplified.meshlet.vertices, simplified.meshlet.indices, 64, 128);
            ...
            const s = getStats(group, splits);
            // console.log(`[split] group.tris: ${s.input}, splits.tris: ${s.output}, group.length: ${group.length}, splits.length: ${splits.length}, split.parts: ${parts}`);
            // break;
        }
        return newMeshlets;
    }
    ...
    function getStats(inputs: Meshlet[], outputs: Meshlet[]): {input: number, output: number} {
        ...
        return {input: inputTriangleCount, output: outputTriangleCount};
    }
