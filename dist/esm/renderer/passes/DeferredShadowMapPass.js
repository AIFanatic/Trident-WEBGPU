...
export class DeferredShadowMapPass extends RenderPass {
    ...
    getCascades(camera, cascadeCount, light) {
        ...
        for (let i = 0; i < cascadeCount; i++) {
            ...
            lightViewMatrix.lookAt(eye, frustumCenter, new Vector3(0, 1, 0));
    ...
    execute(resources) {
        ...
        for (let i = 0; i < meshes.length; i++) {
            const mesh = meshes[i];
            if (!mesh.enabled || !mesh.enableShadows)
                continue;
...
//# sourceMappingURL=DeferredShadowMapPass.js.map