...
class LayoutAssetEvents {
  ...
  static RequestChangeMaterialType = (oldMaterial, newTypeId) => {
  };
}
...
class StringUtils {
  ...
  static NicifyVariableName(name) {
    if (!name) return "";
    return name.replace(/^m_/, "").replace(/^_+/, "").replace(/[_-]+/g, " ").replace(/([a-z0-9])([A-Z])/g, "$1 $2").replace(/([A-Z]+)([A-Z][a-z])/g, "$1 $2").replace(/\s+/g, " ").trim().split(" ").map(
      (word) => word === word.toUpperCase() && word.length <= 3 ? word : word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
    ).join(" ");
  }
}
    ...
    for (const component of go.GetComponents()) {
      ...
      if (component.constructor?.type === ComponentRegistry.Mesh.type || component.constructor?.type === ComponentRegistry.SkinnedMesh.type) {
        ...
        if (material && !material.assetPath) {
          ...
          const safeName = sourceName.replace(/[\\/:*?"<>|#]/g, "_").replace(/\s+/g, "_").replace(/_+/g, "_").replace(/^_|_$/g, "");
...
class InspectorMaterial extends Component {
  ...
  onTypeSelected(newTypeId) {
    const currentType = this.props.material.constructor.type;
  ...
  render() {
    ...
    const currentType = this.props.material.constructor.type;
    const options = Array.from(GPU.Material.Registry.keys()).map((type) => ({ text: type, value: type }));
    return /* @__PURE__ */ createElement("div", null, /* @__PURE__ */ createElement(InspectorProperty, { title: "Type" }, /* @__PURE__ */ createElement(
      InspectorDropdown,
      {
        options,
...
class LayoutInspector extends Component {
  constructor(props) {
    ...
    TridentAPI.EventSystem.on(LayoutAssetEvents.ScriptReloaded, () => {
      this.setState({ ...this.state });
    });
  }
...
class App extends Component {
  ...
  render() {
    return /* @__PURE__ */ createElement(Layout, { engineAPI: EngineAPI });
  }
}
