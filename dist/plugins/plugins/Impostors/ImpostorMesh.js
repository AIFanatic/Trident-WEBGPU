import { Components, Scene, GPU, GameObject, Mathf, Geometry, PBRMaterial } from '@trident/core';
...
class ImpostorMesh extends Components.Mesh {
  ...
  async Create(objects, atlasResolution = 2048, atlasTiles = 12) {
    ...
    camera.SetOrthographic(-radius, radius, radius, -radius, 0, radius * 2);
    ...
    for (var x = 0; x < frames; x++) {
      for (var y = 0; y < frames; y++) {
        ...
        camera.transform.LookAtV1(new Mathf.Vector3(0, 0, -1e-7));
