...
class PostProcessingBloom extends GPU.RenderPass {
  params = {
    threshold: 0.4,
    softThreshold: 0.25,
    strength: 1,
    downsampleDistance: 5,
    upsampleDistance: 0.5,
    iterations: 4
  };
  name = "PostProcessingBloom";
  shader;
  quadGeometry;
  renderTarget;
  currentPassBuffer;
  ...
  getTempTexture(key, width, height) {
  ...
  setPassParams(passIndex, sourceWidth, sourceHeight) {
    const minUniformBufferOffsetAlignment = 256;
  ...
  drawTo(target, label) {
    GPU.RendererContext.BeginRenderPass(label, [{ target, clear: true }], void 0, true);
