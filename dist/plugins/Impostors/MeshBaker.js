...
class MeshBaker {
  static async awaitShaders(meshes) {
    for (const mesh of meshes) void mesh.material.shader;
    await new Promise((r) => requestAnimationFrame(r));
    await new Promise((r) => requestAnimationFrame(r));
  }
  static computeBounds(meshes) {
    ...
    for (const mesh of meshes) {
      const pos = mesh.geometry?.attributes.get("position")?.array;
      if (!pos) continue;
  ...
  static Bake(meshes, modelMatrices, camera, targets, viewport, clearColor = true) {
