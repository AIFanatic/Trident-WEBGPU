import { GPU, EventSystem, Components } from '@trident/core';
import { MeshletEvents, MeshletMeshV3 } from '../MeshletMesh.js';
...
class PrepareMeshletData extends GPU.RenderPass {
  ...
  async preFrame(resources) {
        ...
        for (const lodMeshlet of lodMeshlets) {
          ...
          for (let meshletIndex = 0; meshletIndex < meshlets.length; meshletIndex++) {
            ...
            const meshletInfo = new Float32Array([
              ...
              ...m.bounds.center,
              m.bounds.radius,
              ...m.parentBounds.center,
              m.parentBounds.radius,
              ...m.cone.axis,
              m.cone.cutoff,
              ...m.cone.apex,
              0,
            ...
            this.objectInfoBuffer.set(this.currentMeshletCount, new Uint32Array([
              meshletOffset / meshletInfo.length,
              // meshletIndex
              lodMeshIndex / lodMeshData.length
              // lodMeshIndex 
            ]));
    ...
    const frameBufferData = new Float32Array([
      ...
      0,
      ...frustumPlanes[0].normal.elements,
      frustumPlanes[0].constant,
      ...frustumPlanes[1].normal.elements,
      frustumPlanes[1].constant,
      ...frustumPlanes[2].normal.elements,
      frustumPlanes[2].constant,
      ...frustumPlanes[3].normal.elements,
      frustumPlanes[3].constant,
      ...frustumPlanes[4].normal.elements,
      frustumPlanes[4].constant,
      ...frustumPlanes[5].normal.elements,
      frustumPlanes[5].constant,
      this.currentMeshletCount,
      0,
      GPU.Renderer.width,
      GPU.Renderer.height,
