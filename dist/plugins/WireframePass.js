...
class WireframePass extends GPU.RenderPass {
  name = "WireframePass";
  enabled = true;
  color = [1, 1, 1];
  shader;
  skinnedShader;
  wireframeCache = /* @__PURE__ */ new WeakMap();
  drawList = [];
  colorBuffer = new Float32Array(4);
  skinnedModelMatrices;
  async init() {
    ...
    for (const [, renderable] of Components.Renderable.Renderables) {
      ...
      if (renderable instanceof Components.InstancedMesh) {
        ...
        this.drawList.push({
          wire,
          buffer: buf,
          offset: renderable.modelMatrixOffset * Components.Mesh.modelMatrices.getStride(),
          count: 1
        });
      }
    }
