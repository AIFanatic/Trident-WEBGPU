...
class TerrainEditor extends (_a = Components.Component, _paintObjects_dec = [SerializeField(Prefab)], _paintObjectID_dec = [SerializeField], _paintLayers_dec = [SerializeField(TerrainLayer)], _paintMaterialId_dec = [SerializeField], _paintRadius_dec = [SerializeField], _paintStrength_dec = [SerializeField], _paintTextureStrength_dec = [SerializeField], _paintObjectMinScale_dec = [SerializeField], _paintObjectMaxScale_dec = [SerializeField], _paintObjectDensity_dec = [SerializeField], _heightmap_dec = [SerializeField(GPU.Texture)], _heightmapMultiplier_dec = [SerializeField], _heightmapSmooth_dec = [SerializeField], _editType_dec = [SerializeField(EditType)], _a) {
  ...
  RaycastTerrain() {
    ...
    const hit = collider.castRayAndGetNormal(ray, 1e4, true);
    if (!hit) return null;
      ...
      activeSection = h(
        ...
        h(
          ...
          h(
            "div",
            { className: "row", style: { display: "block" } },
            EditorAPI.LayoutInspectorProperty({ title: "Size X", children: [EditorAPI.LayoutInspectorInput({ value: td.size.x, min: 1, max: 1e4, step: 1, onChanged: (value) => td.Resize(parseFloat(value), td.size.y, td.size.z) })] }),
            EditorAPI.LayoutInspectorProperty({ title: "Size Y", children: [EditorAPI.LayoutInspectorInput({ value: td.size.y, min: 1, max: 1e4, step: 1, onChanged: (value) => td.Resize(td.size.x, parseFloat(value), td.size.z) })] }),
            EditorAPI.LayoutInspectorProperty({ title: "Size Z", children: [EditorAPI.LayoutInspectorInput({ value: td.size.z, min: 1, max: 1e4, step: 1, onChanged: (value) => td.Resize(td.size.x, td.size.y, parseFloat(value)) })] }),
